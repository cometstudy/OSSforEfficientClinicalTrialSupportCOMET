### Created:    May 6, 2021 by Jon Clutton
### Supporting Fitbit API Functions

#' @section Copyright: 
#' Copyright © 2021 University of Kansas


message("Beginning 00_fitbit_functions")



#' @name fitbit_parse
#' 
#' @title Parse the output of a fitbit request
#' 
#' @description This is a generic function that converts the Fitbit API response from a JSON list into a data.table 
#' 
#' @param req Output object of the get_hr request
#' @return List representing the JSON content of the get_hr response
#' 
fitbit_parse <- function(get_hr = NULL) {
  text <- content(get_hr, as = "text")
  if (identical(text, "")) stop("No output to parse", call. = FALSE)
  jsonlite::fromJSON(text, simplifyVector = FALSE)
}

#' @name jsonlist_to_df
#' 
#' @title Simply convert json-as-list to dataframe
#' 
#' @description 
#' This is a generic function that converts a JSON list to a data.frame
#' 
#' @param data JSON as list output from a GET request to Fitbit API
#' @return data.frame of JSON list
#' 
########## json-as-list to dataframe (for simple cases without nesting!) function #######
jsonlist_to_df <- function(data = NULL) {
  purrr::transpose(data) %>%
    purrr::map_dfr(., unlist) %>% 
    as.tibble(., .name_repair = c( "unique"))
}

#' @name get_status
#' 
#' @title Get status
#' 
#' @description 
#' This is a fitbit function that retrieves whether a fitbit token is active. 
#' Success
#' 200 OK
#' 201 Created
#' 204 No Content
#' Client Errors
#' 400 Bad Request
#' 401 Unathorized
#' 403 Forbidden
#' 404 Not Found
#' 409 Conflict
#' 411 Length Required
#' 429 Too many requests
#' Server Errors
#' 500 Internal Server Error
#' 502 Bad Gateway
#' 503 Service Unavailable
#' 504 Gateway Timeout
#' 
#' @param usercode The fitbit usercode associated with a user's account
#' @param token The unique access_token for Fitbit API access, access_token must be renewed every 8 hours
#' @return status
#' 
get_status <- function(usercode = NULL, token = NULL) {
  last_status <- GET(url =
                                  paste0('https://api.fitbit.com/1/user/',
                                         usercode,
                                         '/devices.json'),
                                add_headers(Authorization = paste0("Bearer ", token)))
  
  status <- last_status$status_code
  return(status)
  
}

#' @name get_last_sync
#' 
#' @title Get last sync
#' 
#' @description This is a fitbit function that retrieves the last sync time of a given fitbit device
#' 
#' @param usercode The fitbit usercode associated with a user's account
#' @param token The unique access_token for Fitbit API access, access_token must be renewed every 8 hours
#' @return last_sync
#' @return battery
#' 
#' @section Development
#' 20230323: Updated get last sync to remove any MobileTrack devices on accounts. JC
#' 
get_last_sync <- function(usercode = NULL, token = NULL) {
  last_sync_json <- content(GET(url =
                                      paste0('https://api.fitbit.com/1/user/',
                                             usercode,
                                             '/devices.json'),
                                    add_headers(Authorization = paste0("Bearer ", token))))
  
  last_sync_df <- jsonlist_to_df(last_sync_json) %>%
    filter(deviceVersion != "MobileTrack")
  
  last_sync <- last_sync_df$lastSyncTime[1]
  #battery <- last_sync_df$batteryLevel[1]
  return(last_sync)
  #return(battery)

  
}

#' @name get_device_data
#' 
#' @title Get Device Info
#' 
#' @description This is a fitbit function that returns all device related data 
#' 
#' @param usercode The fitbit usercode associated with a user's account
#' @param token The unique access_token for Fitbit API access, access_token must be renewed every 8 hours
#' @return device_data

#' 
get_device_data <- function(usercode = NULL, token = NULL) {
  last_sync_json <- content(GET(url =
                                  paste0('https://api.fitbit.com/1/user/',
                                         usercode,
                                         '/devices.json'),
                                add_headers(Authorization = paste0("Bearer ", token))))
  
  device_data <- jsonlist_to_df(last_sync_json) %>%
    filter(deviceVersion != "MobileTrack") %>%
    filter(type == "TRACKER")
  
  return(device_data)

}

#' @name get_battery_level
#' 
#' @title Get battery level
#' 
#' @description This is a fitbit function that retrieves the battery of a fitbit device
#' 
#' @param usercode The fitbit usercode associated with a user's account
#' @param token The unique access_token for Fitbit API access, access_token must be renewed every 8 hours
#' @return battery
#' 
get_battery_level <- function(usercode = NULL, token = NULL) {
  last_sync_json <- content(GET(url =
                                  paste0('https://api.fitbit.com/1/user/',
                                         usercode,
                                         '/devices.json'),
                                add_headers(Authorization = paste0("Bearer ", token))))
  
  last_sync_df <- jsonlist_to_df(last_sync_json) %>%
    filter(deviceVersion != "MobileTrack")
  
  battery <- last_sync_df$batteryLevel[1]
  return(battery)
  
}

#' @name get_steps
#' 
#' @title Get Steps Request
#' 
#' @description This is a specific COMET function. It makes a fitbit API request for steps per day and returns a data.table of steps per day. 
#' 
#' @param start_date The first date of the request
#' @param end_date The last date of the request
#' @param usercode The fitbit usercode associated with a user's account
#' @param token The unique access_token for Fitbit API access, access_token must be renewed every 8 hours
#' 
#' @return data.table of steps per day
#' 
### Get steps request from Fitbit API in 1 d increment function #####
get_steps <- function(start_date = NULL, end_date = NULL,  usercode = NULL,
                      token = NULL) {
  steps_json <- GET(url =
                          paste0('https://api.fitbit.com/1/user/',
                                 usercode,
                                 '/activities/steps/date/',
                                 start_date,
                                 '/',
                                 end_date,'.json'),
                        add_headers(Authorization = paste0("Bearer ", token)))
  
  steps <- jsonlist_to_df(content(steps_json))
  return(steps)
}



#' @name get_hr
#' 
#' @title Get Intraday HR Request
#'
#' @description This is a specific COMET function. It makes a fitbit API request for one day's worth of intraday HR data. The data is returned in JSON format and must be parsed.
#' 
#' @param date The date of the request
#' @param usercode The fitbit usercode associated with a user's account
#' @param token The unique access_token for Fitbit API access, access_token must be renewed every 8 hours
#' 
#' 
#' @return data.frame of intraday HR activity
 
########## Get hr request from Fitbit API in 1 m increment function  ######
get_hr <- function(date = NULL,  usercode = NULL,
                   token = NULL) {
  
  request <- paste0('https://api.fitbit.com/1/user/',
                    usercode,
                    '/activities/heart/date/',
                    date,
                    '/1d/1min/time/00%3A00/23%3A59.json')
  
  got_hr <- GET(url =
                        request,
                      add_headers(Authorization = paste0("Bearer ", token)))
  
  
  
  hr_json <- fitbit_parse(got_hr)
  
  date <- hr_json$`activities-heart`[[1]]$dateTime
  
  hr_list <- hr_json[[2]][[1]]
  
  hr <- data.table::rbindlist(lapply(hr_list, data.table::as.data.table)) %>%
    mutate(date = date)
  
  return(hr)
  
}


#' @name filter_exercise_forloop
#' 
#' @title Filter HR using for loop method
#'
#' @description 
#' This is a comet specific function to filter for exercise data. This method uses a for loop.T
#' The function filters for all values above the moderate hr. It then iteratively builds 
#' timeblocks counting how many minutes the df was above the moderate hr. If the df fluctuates
#' above and below moderate hr, the timeblock continues to count as long as the lapse does
#' not last longer than 5 minutes.
#'
#' 
#' @param df Data frame of one day's worth of time and hr data
#' @param moderate the heart rate value of a person's moderate intensity exercise
#' 
#' 
#' @return number of minutes of exercise according to for-loop method
#' 
#' 
filter_exercise_forloop <- function(df = NULL, moderate = NULL) {
  
  df <- filter(df, value >= moderate) %>%
    mutate(timeblock = 0) %>%
    mutate(time = hms::as_hms(time))
  
  if(nrow(df)<10){
    return (0)
  } else {
    temp_timeblock=0
    for(j in 2:nrow(df))
    {
      if(df$time[j]-df$time[j-1]<=300)
      {
        temp_timeblock = temp_timeblock + as.numeric((df$time[j]-df$time[j-1])/60)
        df$timeblock[j]= temp_timeblock
        df$timeblock[j-1]=0
      }
      if(df$time[j]-df$time[j-1]>300)
      {
        temp_timeblock = 1
        
      }
    }
    
    df2 <- df %>%
      filter(timeblock >= 10)
    
    ex_forloop = sum(df2$timeblock)
    
    return(ex_forloop)
  }

}

#' @name filter_exercise_averaging
#' 
#' @title Filter HR using averaging
#'
#' @description This is a comet specific function to filter for exercise data. This method uses an average of every
#' five minutes. All five minute blocks above a pt's moderate exercise are kept. 
#'
#' 
#' @param df Data frame of one day's worth of time and hr data
#' @param moderate the heart rate value of a person's moderate intensity exercise
#' 
#' 
#' @return number of minutes of exercise according to averaging method
#' 
#' 
filter_exercise_averaging <- function(df = NULL, moderate = NULL) {
  
  df <- mutate(df, datetime = as_datetime(paste(today()-1,time))) %>%
    group_by(datetime = cut(datetime, breaks="5 min")) %>%
    summarize(value = mean(value)) %>%
    filter(value >= moderate)
  
  ex_averaging <- nrow(df)*5
  
  return(ex_averaging)
}

#' @name get_activities
#' 
#' @title Get Activities
#' 
#' @description 
#' This is a specific COMET function. It makes a fitbit API request for a day's activity
#' and returns a data.frame of all activities that day 
#' 
#' @param date The date of the request
#' @param usercode The fitbit usercode associated with a user's account
#' @param token The unique access_token for Fitbit API access, access_token must be renewed every 8 hours
#' 
#' @return data.table of activites
#' 
#' @section Updates
#' 20230323: Changed how to delist activities. One participant had some parts of the list (distance) return as null. 
#' So, I removed the distance section of activities before delisting. JC
#' 
get_activities <- function(date = NULL,  usercode = NULL,
                      token = NULL) {
  request <- paste0('https://api.fitbit.com/1/user/',
                    usercode,
                    '/activities/date/',
                    date,
                    '.json')
  
  got_activities <- GET(url =
                          request,
                        add_headers(Authorization = paste0("Bearer ", token)))
  
  activities_list <- fitbit_parse(got_activities)
  
  #activities <- jsonlist_to_df(activities_list$activities)
  
  activities <- purrr::transpose(activities_list$activities) %>%
    purrr::list_modify("distance" = NULL) %>%
    purrr::map_dfr(., unlist) %>%
    as.tibble(., .name_repair = c( "unique"))
  
  return(activities)
}

#' @name on_clip_error_inspire
#' 
#' @title On Clip Error Email
#' 
#' @description 
#' Sometimes a setting on the Inspire 2's gets changed from on-wrist to on-clip.
#' When this happens, HR is not collected. This function will send an email to participants
#' with instructions how to fix. 
#' 
#' @param fname participant's fist name
#' @param email participant's email
#' 
#' @section Development
#' 5.20.22 Started development JC \cr
#' 9.22.22 Changed name to on_clip_inspire JC \cr

on_clip_error_email_inspire <- function(fname = NULL, email = NULL) {
  
  
  body <- paste0('<!DOCTYPE html>
                  <table align="center" border="0" cellpadding="0" cellspacing="0" width="600">
                    <tr>
                      <td align="center" style="padding: 40px 0 15px 0;">
<img src="data:image/jpeg;base64,
" />

                   </td>
                    </tr>
                    <tr>
                      <td align="left" style="padding: 0px 0 0px 0;">
                        Hi ',
                 fname,
                 '! <br><br> We noticed a potential issue with your Fitbit watch. It looks like it might not be collecting heart rate. This is a common problem. We have outlined how to fix it below. Please call us if you have any issues. <b> Study Coordinator at coordinator_number.</b> or <b> Jon Clutton at 913-945-7683.</b> <br> <br>
                      </td>
                    </tr>
                  <tr>
                    <td align="left">
                      <p style= "font-size:100%;"> 1) Take your watch off and see if there is a blinking green light. If not, continue to step 2. If there is a green light, call Jon Clutton. <br>
                                                   2) Hold both sides of your Inspire 2 Fitbit for 2-3 seconds until a menu appears starting with "Water Lock"<br>
                                                   3) Scroll down to the bottom of the menu and look for "On Clip" or "On Wrist." If the watch says "On Clip," continue to step 4. If the watch says "On Wrist," call Jon Clutton. <br>
                                                   4) Select "On Clip," scroll down and select the check mark <br>
                                                   5) Take your watch off again and check for the blinking green light. It should be there now. If so, problem solved! Nicely done!<br><br></p>
                    </td>
                   </tr>
                  <tr>
                    <td align="left">
                      <p style= "font-size:100%;"> If you need a visual tutorial, this video should explain how to fix the problem. <br>
                                                   Video link: https://bit.ly/fitbit_no_hr_error</p>
                    </td>
                   </tr>

                    ')
  
  cat(body, file = file.path(data_dir,'modules','participant_error','email.txt'))
}

#' @name on_clip_error_charge
#' 
#' @title On Clip Error Email
#' 
#' @description 
#' Sometimes a setting on the Charge 5's turns of HR collection. This function will send an email to participants
#' with instructions how to fix. 
#' 
#' @param fname participant's fist name
#' @param email participant's email
#' 
#' @section Development
#' 9.22.22 Started development. Adapting from on_clip_inspire. JC \cr

on_clip_error_email_charge <- function(fname = NULL, email = NULL) {
  
  
  body <- paste0('<!DOCTYPE html>
                  <table align="center" border="0" cellpadding="0" cellspacing="0" width="600">
                    <tr>
                      <td align="center" style="padding: 40px 0 15px 0;">
<img src="data:image/jpeg;base64,
" />

                   </td>
                    </tr>
                    <tr>
                      <td align="left" style="padding: 0px 0 0px 0;">
                        Hi ',
                 fname,
                 '! <br><br> We noticed a potential issue with your Fitbit watch. It looks like it might not be collecting heart rate. This is a common problem. We have outlined how to fix it below. Please call us if you have any issues. <b> Study Coordinator at coordinator_number.</b> or <b> Jon Clutton at 913-945-7683.</b> <br> <br>
                      </td>
                    </tr>
                  <tr>
                    <td align="left">
                      <p style= "font-size:100%;"> 1) Take your watch off and see if there is a blinking green light. If not, continue to step 2. If there is a green light, call Jon Clutton. <br>
                                                   2) Open up your watch screen and scroll down (drag your finger down) and press SETTINGS.<br>
                                                   3) Scroll down to HEART RATE. If the bar is grey and to the left, continue to step 4. If the bar is blue and to the right, call Jon Clutton. <br>
                                                   4) Press Heart RATE and you should see the bar move to the right and turn blue. <br>
                                                   5) Take your watch off again and check for the blinking green light. It should be there now. If so, problem solved! Nicely done!<br><br></p>
                    </td>
                   </tr>
                  <tr>
                    <td align="left">
                      <p style= "font-size:100%;"> If you need a visual tutorial, this video should explain how to fix the problem. <br>
                                                   Video link: https://bit.ly/fitbit_no_hr_error_charge</p>
                    </td>
                   </tr>

                    ')
  
  cat(body, file = file.path(data_dir,'modules','participant_error','email.txt'))
}
